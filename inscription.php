...
$bdd = new PDO('mysql:host=localhost;dbname=CRM;charset=utf8', 'root', 'root', array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
//$reponse = $bdd->query('SELECT * FROM membres'); 

if(isset($_POST['pseudo']) && isset($_POST['email']) && isset($_POST['pass']) && isset($_POST['pass_confirm']))
{

	$req_pseudo = $bdd->prepare('SELECT pseudo FROM members WHERE pseudo = ?');
	$req_pseudo->execute(array(htmlspecialchars($_POST['pseudo'])));
	$verif_pseudo = $req_pseudo-> fetch();

	$req_email = $bdd->prepare('SELECT email FROM members WHERE email = ?');
	...
	else
	{
		...
		$req = $bdd->prepare('INSERT INTO members (pseudo, email, pass) VALUES(:pseudo, :email, :pass)');
		$req->execute(array(
...
<body>
	<p><a href="/">Connectez-vous ici</a> ou inscrivez-vous ci-dessous :</p>
